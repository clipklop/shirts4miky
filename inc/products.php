...
function get_products_all() {
  ...
  # It's a good idea to have a separate try catch block for each point of interaction.
  try {
  ...
  # Fetching all products in an associative array
...
/*
 * Returns an array of proudct information for the product that matches the sku
...
function get_product_single($sku) {
  ...
  try {
    $results = $db->prepare("SELECT name, price, img, sku FROM products WHERE sku = ?");
  ...
  if ($product === false) {
    ...
    while ($row = $results->fetch(PDO::FETCH_ASSOC)) {
      # Looping rows until it gets false in $results
